...
def deconv_slice(data):
   ...
   # delta = 10  
    ...
    #--------------------------------------------------------------------------------#
    OTF_SL = OTF[0:20,:]
    H = np.concatenate((OTF_SL,np.zeros((20,600))),axis=0)
    #h = PSF
   ...
   # R = np.fft.fft2(refr)
    ...
    #------------------------------------------------#
    noise = 0.1*np.random.rand(40,Fs) - 0.05
    ...
    while (diff>=.0001):
        up = np.conj(H)
        down = .00000001 + abs(H)*abs(H) + 10.5*(np.conj(Dx1)*Ax1*Dx1 + np.conj(Dy1)*Ay1*Dy1 + np.conj(Dx2)*Ax2*Dx2 + np.conj(Dy2)*Ay2*Dy2 +  np.conj(Dcross1)*Across1*Dcross1 + np.conj(Dcross2)*Across2*Dcross2)
